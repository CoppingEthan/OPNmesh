...
export function ClientDetail({ initial, row }: { initial: StatePayload; row: ClientRowView }) {
  ...
  const canConnect = state.sites.some((s) => s.reachable) && row.enabled && !held;

  useEffect(() => {
    apiFetch<{ pending: { expiresAt: number } | null }>("GET", `/api/admin/clients/${row.id}/invite`)
      .then((r) => setPending(r.pending))
      .catch(() => setPending(null));
  }, [row.id]);

  const cancelLink = async () => {
    await apiFetch("DELETE", `/api/admin/clients/${row.id}/invite`);
    setInvite(null);
    setPending(null);
  };
          ...
          <>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                // Disabling cancels unused links on the server.
                if (row.enabled) {
                  setInvite(null);
                  setPending(null);
                }
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <div className="space-y-6 xl:col-span-2">
          <Card title="Hand it over" description="Three ways to get the configuration onto the device. Each contains the private key, so treat it like a password.">
            {held && row.enabled ? (
              <Callout tone="warn" title="Configuration on hold">
                {held}. Until this is fixed, this client&apos;s configuration cannot be handed out.
              </Callout>
            ) : (
              !canConnect && (
            ...
            <div className="mt-4 grid gap-6 md:grid-cols-[220px_1fr]">
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <p className="mb-2 text-xs text-ink-2">The person opens the link and collects the QR code and file themselves. It works once, expires after 24 hours, and a new link cancels the previous one.</p>
                  {invite ? (
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <Button
                        ...
                        disabled={!canConnect}
                        onClick={async () => {
                          setInvite(await apiFetch("POST", `/api/admin/clients/${row.id}/invite`, {}));
                          setPending(null);
                        }}
                      >
                        {pending ? "Create a new link" : "Create link"}
                      </Button>
                      {pending && (
                        <>
          ...
          <Card title="Keys and removal">
            <div className="flex flex-col items-start gap-2">
              <ConfirmButton
                ...
                variant="secondary"
                description="Generates a new key pair. Every copy of the current configuration, and any invite link not yet used, stops working immediately; hand over a new one afterwards."
                onConfirm={async () => {
              ...
              <ConfirmButton
                label="Delete client"
                description="Removes the device from every gateway within seconds and cancels any unused invite link. This cannot be undone."
                onConfirm={async () => {
